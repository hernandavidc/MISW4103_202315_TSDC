...
describe('Create a post', () => {
  ...
  it('1.3 Publicar un post programado y verlo en la lista de filtro', () => {
    cy.custom_screenshot('1.3-1'); // llegar a la pagina
    // when
    const postTitle = faker.lorem.sentence();
    const postContent = faker.lorem.paragraphs(2);
    cy.get('textarea[placeholder="Post title"]').type(postTitle);
    cy.get('.kg-prose > p').type(postContent);
    cy.custom_screenshot('1.3-2');

    // Se define la URL
    cy.get('.settings-menu-toggle').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-3');
    cy.get('input#url').type('-blog-test');
    cy.get('.settings-menu-toggle').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-4');

    // Proceso para confirmar la publiación del post
    cy.get('.gh-publish-trigger').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-5');
    cy.get('[data-test-setting="publish-at"] button').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-6');
    cy.get('.gh-radio').contains('Schedule for later').click();
    cy.get('button').contains('Continue').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-7');
    cy.get('.gh-publish-cta button').contains('Publish').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-8');
    cy.get('[data-test-button="close-publish-flow"]').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-9');
    cy.get('a[href="#/posts/"][data-test-breadcrumb]').click();
    cy.wait(1000);
    cy.custom_screenshot('1.3-10');
  ...
  it('1.4 Intentar publicar el post pero en el formulario de publicar cancelar', () => {
    cy.custom_screenshot('1.4-1'); // llegar a la pagina
    // when
    const postTitle = faker.lorem.sentence();
    const postContent = faker.lorem.paragraphs(2);
    cy.get('textarea[placeholder="Post title"]').type(postTitle);
    cy.get('.kg-prose > p').type(postContent);
    cy.custom_screenshot('1.4-2');

    // Proceso para confirmar la publiación del post
    cy.get('.gh-publish-trigger').click();
    cy.wait(1000);
    cy.custom_screenshot('1.4-3');
    cy.get('button').contains('Continue').click();
    cy.wait(1000);
    cy.custom_screenshot('1.4-4');
  
    // Cancelar el proceso de publicación
    cy.get('[data-test-button="close-publish-flow"]').click();
    cy.wait(1000);
    cy.custom_screenshot('1.4-5');
    cy.get('a[href="#/posts/"][data-test-breadcrumb]').click();
    cy.wait(1000);
    cy.custom_screenshot('1.4-6');
    cy.get('a[href="#/posts/?type=draft"]').click();
    cy.wait(1000);
    cy.custom_screenshot('1.4-7');
